...
| `node`/`npx` abort with `Library not loaded: .../libada.3.dylib` (breaks `npx gnhf`, and `opencode`/`oc` which is a Homebrew node dependent) | Homebrew `node` links `libada.3`, but its `ada-url` dependency auto-upgraded to 4.0.0 (only `libada.4` exists) and node wasn't relinked - Homebrew `onActivation.autoUpdate = true` is the trigger | `brew reinstall node` pulls a current node bottle linked to `libada.4`. Do NOT `brew uninstall node`: `opencode` depends on it. Note this node is undeclared cruft - you declare nixpkgs `nodejs_22` (works at v22, shadowed on PATH), so a rebuild would zap it anyway |
| `brew reinstall/install` aborts with `Error: unknown install step: remove` | nix-homebrew's patched brew (6.0.1, `zhaofengli/nix-homebrew`) is behind homebrew-core; the current formula uses a post-install step the patched brew doesn't implement | the bottle itself usually still pours and links (node came out usable), so verify with `node -v` before panicking. Durable fix: `nix flake update nix-homebrew && ./rebuild.sh` - never mid-brew |
| `gnhf` (or any `oc`/OpenCode agentic run) burns iterations with "OpenCode produced no final answer", 0 commits, then bails after 3 strikes - even though the omlx server is up and answers `curl` | the model was pointed at a **reasoning** model (`Qwen3.6-35B-A3B`), which spends its whole output budget in a `<think>` stream and never lands a clean tool call. Proof: `curl` the model with "reply exactly OK" and it returns `"Here's a thinking process:\n\n1."` with `finish_reason: length` instead of `OK`/`stop`. Reasoning models are fine for chat, wrong for agentic tool-calling loops | point OpenCode at a **coder** model instead - `~/.config/opencode/opencode.json` `"model": "omlx/mlx-community--Qwen3-Coder-Next-8bit"` (declare it under `provider.omlx.models` too). Verify: `curl` it with "reply exactly OK" - a coder model returns `OK` / `finish_reason: stop`. Same rule for Codex/`co`, which already defaults to the Coder model (see §5). First call per model is a ~1min cold MLX load, not a hang |
